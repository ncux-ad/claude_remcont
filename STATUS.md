...
### v0.9 — /history SESSION_ID, session_id в задачах, DEPLOY.md
...
### v0.10 — code review fixes (текущий)

По итогам ревью v0.9 (4 находки, все закрыты):

- **`/history SESSION_ID` по префиксу** (#1): бот показывает id усечённым до 12 символов, а поиск шёл по точному совпадению — введённый id не находился. Добавлен `session_manager.resolve(prefix, chat_id)` (поиск по префиксу через `LIKE` с экранированием wildcard-символов, в т.ч. `_`); `/history`, `/session`, `/label` резолвят префикс и сообщают о неоднозначности.
- **Бэкфилл `session_id`** (#2): для `/new` и неявного `--continue` сессия неизвестна при постановке в очередь. `queue_manager.set_session_id()` проставляет реальную сессию в `run_claude`, как только Claude её резолвит — задача попадает в `/history <sid>`.
- **Миграция один раз за процесс** (#3): `_migrate()` с double-checked locking (отдельный лок, без дедлока с `_lock`), проверка колонки через `PRAGMA table_info` вместо исключения-как-управления.
- **Потеря данных при апгрейде** (#4): `idx_session` ссылался на `session_id` и стоял в `_SCHEMA` (выполняется до `ALTER`), из-за чего на старой БД `executescript()` падал → ловилось как «БД повреждена» → очередь удалялась. Индекс перенесён в `_migrate` (после `ALTER`).

**Тесты: 68 тестов, 100% pass** (+9: prefix-resolve, бэкфилл, апгрейд старой БД без потери задач)
